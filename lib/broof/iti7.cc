...
void count_object::add(count_object* c){
  ...
  while(it_cc != class_counts.end()){
    std::map<std::string, int>::iterator it_cc_current = c->class_counts.find(it_cc->first);
    if(it_cc_current == c->class_counts.end()){
      class_counts.insert(std::pair<std::string, int>(it_cc->first, it_cc->second));
    }
...
void variable_object::add(variable_object* v){
  ...
  while(it_cc != class_counts.end()){
    std::map<std::string, int>::iterator it_cc_current = v->class_counts.find(it_cc->first);
    if(it_cc_current == v->class_counts.end()){
      class_counts.insert(std::pair<std::string, int>(it_cc->first, it_cc->second));
    }
  ...
  it_vc = value_counts.begin();
  while(it_vc != value_counts.end()){
    std::map<double, count_object*>::iterator it_vc_current = v->value_counts.find(it_vc->first);
    if(it_vc_current == v->value_counts.end()){
      count_object * co = new count_object();
      it_vc_current = value_counts.insert(std::pair<double, count_object*>(it_vc->first, co)).first;
      it_vc_current->second->add(it_vc->second);
    }
...
Node::Node(Node*& l, Node*& r){
  ...
  while(it_cc != class_counts.end()){
    std::map<std::string, int>::iterator it_cc_current = r->class_counts.find(it_cc->first);
    if(it_cc_current == r->class_counts.end()){
      class_counts.insert(std::pair<std::string, int>(it_cc->first, it_cc->second));
    }
  ...
  it_v = variables.begin();
  while(it_v != variables.end()){
    std::map<std::string, variable_object*>::iterator it_v_current = r->variables.find(it_v->first);
    if(it_v_current == r->variables.end()){
      variable_object * var = new variable_object();
      it_v_current = variables.insert(std::pair<std::string, variable_object*>(it_v->first, var)).first;
      it_v_current->second->add(it_v->second);
    }
    ...
    if(right){
      ...
      if(left->left || left->right){
        ...
        if(right->right || right->left){
          ...
          std::list<instance_object*>::iterator it_il = left_instances.begin();
