  ...
  void parse_result(const std::string &fn) {
    ...
    if (file) {
      ...
      while (std::getline(file, line)) {
        ...
        // true class is a comma separated list of classes
        ...
        std::string real_classes = tokens[1];
        ...
        Utils::string_tokenize(real_classes, real, ",");
        for (unsigned int i = 0; i < real.size(); i++) {
          stats_.back().add_prediction(real[i], pred);
        }
      }
      file.close();
      stats_.back().summary();
