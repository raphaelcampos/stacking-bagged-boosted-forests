...
class TwoPassClassifier : public SupervisedClassifier {
 public:
  TwoPassClassifier(SupervisedClassifier *fst,
                    TemporalClassifier *tmp, unsigned int r=0, double b=1.0) :
    SupervisedClassifier(r),
    fst_(FirstPassClassifier(fst)), tmp_cl_(tmp),
    twf_(".estimatedTWF"),
    trn_a_(".trn1"), trn_b_(".trn2"), inter_(".interm") { tmp->set_beta(b); }
...
void TwoPassClassifier::estimate_twf() {
  ...
  if (file) {
    ...
    while (std::getline(file, line)) {
      ...
      // format: doc_id CLASS=true_year CLASS=pred_year:value
      ...
      if (tokens.size() > 2) {
        ...
        int p_r = atoi(tokens[1].data());  // reference time point
        tokens.clear();
    ...
    Outputer *tmp = get_outputer();
    Outputer *newOut = new TwoPassOutputer(twf_);
    set_outputer(newOut);
